...
{
    Logger::log("Populating map", Logger::INFO);
    ...
    Logger::log("Map populated", Logger::INFO);
    updateCases();
}
...
Case *Map::getCase(int x, int y)
{
    if (x >= 0 && x < lineCount && y >= 0 && y < columnCount)
    {
        ...
        if (c == nullptr)
        {
            Logger::log("Error: getCase returned null", Logger::ERROR);
        }
        return c;
    }
    Logger::log("Error: getCase out of bounds : " + std::to_string(x) + " " + std::to_string(y) + " / " + std::to_string(lineCount) + " " + std::to_string(columnCount), Logger::ERROR);
    return nullptr;
}
